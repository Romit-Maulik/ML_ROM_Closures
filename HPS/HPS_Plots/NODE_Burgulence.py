...
for i in range(3):
    ...
    sv = r'$a_'+str(i)+'$'
    ax[i].set_ylabel(sv,fontsize=16)
    sv = r'Mode '+str(i+1)
    ax[i].set_title(sv,fontsize=16)
...
def visualize(mode='train'):
    ...
    ln1, = ax[0].plot(time_array[:],predicted_states[:,0],label='NODE',color='orange')
    ln2, = ax[1].plot(time_array[:],predicted_states[:,1],label='NODE',color='orange')
    ln3, = ax[2].plot(time_array[:],predicted_states[:,2],label='NODE',color='orange')
...
plt.plot(kref, eref, '--', label=r'$k^{-2}$ Reference')
...
plt.loglog(kx_plot, espec2,label='GP',color='green')
plt.loglog(kx_plot, espec3,label='LSTM',color='black')
plt.loglog(kx_plot, espec4,label='NODE',color='orange')
plt.legend()
...
plt.figure()
kx_plot = np.array([float(i) for i in list(range(0, nx // 2))])
plt.loglog(kx_plot, np.abs(espec2-espec1),label='GP-Residual',color='green')
plt.loglog(kx_plot, np.abs(espec3-espec1),label='LSTM-Residual',color='black')
plt.loglog(kx_plot, np.abs(espec4-espec1),label='NODE-Residual',color='orange')
plt.legend()
plt.xlabel(r'$k$',fontsize=16)
plt.ylabel(r'$E(k)$',fontsize=16)
plt.show()
